...
public class Team100CameraServer {
	...
	private static Team100CameraServer server;

	public static Team100CameraServer getInstance() {
		if (server == null) {
			server = new Team100CameraServer();
		}
		return server;
	}
	...
	public void startAutomaticCapture(String cameraName) {
		try {
			...
			DriverStation.reportError("Error when starting the camera: "
					+ cameraName + " " + ex.getMessage(), true);
		}
	}
		...
		while (true) {
			...
			try {
				...
				DriverStation.reportError(
						"Error when getting image from the camera: "
								+ ex.getMessage(), true);
				if (dataBuffer != null) {
					synchronized (this) {
		...
		while (true) {
			try {
				...
				if (compression != kHardwareCompression) {
					DriverStation.reportError(
							"Choose \"USB Camera HW\" on the dashboard", false);
					s.close();
					continue;
				}
				...
				while (true) {
					...
					byte[] imageArray = new byte[imageData.data.getBuffer()
							.remaining()];
					imageData.data.getBuffer().get(imageArray, 0,
							imageData.data.getBuffer().remaining());
					...
					try {
						...
						if (imageData.data.getBuffer() != null) {
							synchronized (this) {
								m_imageDataPool.addLast(imageData.data
										.getBuffer());
							}
						}
					}
